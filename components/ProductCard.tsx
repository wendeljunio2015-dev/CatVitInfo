...
export default function ProductCard({ product }: { product: Product }) {
 const { addItem }=useCart(); const {isFavorite,toggleFavorite}=useFavorites(); const unavailable=product.stockStatus==="indisponivel"; const favorite=isFavorite(product.id); const [added,setAdded]=useState(false); const feedbackTimer=useRef<ReturnType<typeof setTimeout>|null>(null);
 const handleAdd=()=>{addItem(product);setAdded(true);if(feedbackTimer.current)clearTimeout(feedbackTimer.current);feedbackTimer.current=setTimeout(()=>setAdded(false),1800);};
 return <article className="relative flex h-full flex-col rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
  ...
  <div className="flex flex-1 flex-col"><div className="flex flex-wrap gap-2"><span className="rounded-full bg-zinc-800 px-3 py-1 text-xs text-zinc-300">{product.category}</span>{product.badge&&<span className={`rounded-full px-3 py-1 text-xs font-bold ${badgeStyle[product.badge]}`}>{product.badge}</span>}</div>
   ...
   <div className="mt-auto pt-5"><p className="text-3xl font-black text-blue-500">{money.format(product.price)}</p><p className="mt-2 text-sm text-zinc-400">{product.warranty?`Garantia: ${product.warranty}`:"Consulte a garantia"}</p><p className={`mt-1 text-sm font-semibold ${product.stockStatus==="ultimas_unidades"?"text-amber-400":product.stockStatus==="indisponivel"?"text-red-400":"text-zinc-300"}`}>{stockLabel[product.stockStatus]}{product.stockQuantity!=null&&product.stockQuantity>0?` • ${product.stockQuantity} un.`:""}</p>
    <div className="mt-5 grid gap-2"><Link href={`/produto/${product.slug}`} className="rounded-xl border border-zinc-700 px-4 py-3 text-center font-bold hover:bg-zinc-800">Ver detalhes</Link><button disabled={unavailable} onClick={handleAdd} aria-live="polite" className={`w-full rounded-xl px-4 py-3 font-bold transition-all duration-200 disabled:cursor-not-allowed disabled:bg-zinc-700 disabled:text-zinc-400 ${added?"scale-[1.02] bg-emerald-600 text-white shadow-lg shadow-emerald-950/30":"bg-blue-600 text-white hover:bg-blue-500"}`}>{unavailable?"Indisponível":added?"✓ Adicionado ao carrinho":"Adicionar ao carrinho"}</button></div>
   </div></div>
 </article>;
}
