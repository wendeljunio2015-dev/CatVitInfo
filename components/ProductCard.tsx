      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="mt-auto pt-5">
          ...
          <div className="mt-5 grid gap-2">
            <Link href={`/produto/${product.slug}`} className="rounded-xl border border-zinc-700 px-4 py-3 text-center font-bold hover:bg-zinc-800">Ver detalhes</Link>
            <button disabled={unavailable} onClick={() => addItem(product)} className="w-full rounded-xl bg-blue-600 px-4 py-3 font-bold disabled:cursor-not-allowed disabled:bg-zinc-700 disabled:text-zinc-400 hover:bg-blue-500">{unavailable ? "Indisponível" : "Adicionar ao carrinho"}</button>
          </div>
        </div>
      </div>
    </article>
