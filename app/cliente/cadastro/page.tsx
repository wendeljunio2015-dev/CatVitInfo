...
export default async function CustomerRegisterPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <main className="mx-auto max-w-xl px-6 py-16">
    <div className="rounded-3xl border border-zinc-800 bg-zinc-900 p-7">
      ...
      {error ? <p className="mt-5 rounded-xl border border-red-500/20 bg-red-500/10 p-3 text-sm font-bold text-red-300">Não foi possível criar sua conta. Confira os dados e tente novamente.</p> : null}
      <form action="/api/customer/register" method="post" className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <div><label className="text-sm font-bold">Senha</label><input required type="password" name="password" minLength={6} className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3" /></div>
