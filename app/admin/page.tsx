...
import AdminProductImageButton from "@/components/AdminProductImageButton";
...
export default async function AdminPage() {
  ...
  return <main className="mx-auto max-w-7xl px-6 py-12">
    ...
    <section className="mt-10 rounded-2xl border border-zinc-800 bg-zinc-900 p-6"><h2 className="text-2xl font-black">Cadastrar produto</h2><form action="/api/admin/products" method="post" encType="multipart/form-data" className="mt-6 grid gap-4 md:grid-cols-2"><input required name="name" placeholder="Nome do produto" className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3"/><input required name="category" placeholder="Categoria" className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3"/><input required name="price" type="number" min="0" step="0.01" placeholder="Preço" className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3"/><input name="warranty" placeholder="Garantia (opcional)" className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3"/><select name="stockStatus" className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3"><option value="em_estoque">Em estoque</option><option value="ultimas_unidades">Últimas unidades</option><option value="indisponivel">Indisponível</option></select><select name="badge" className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3"><option value="">Sem selo</option><option>Novo</option><option>Promoção</option><option>Destaque</option></select><AdminImageInput/><input name="image" placeholder="Ou cole uma URL da imagem (opcional)" className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 md:col-span-2"/><textarea name="description" placeholder="Descrição" rows={4} className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 md:col-span-2"/><label className="flex items-center gap-3"><input type="checkbox" name="featured"/> Mostrar como destaque</label><button disabled={!databaseReady} className="rounded-xl bg-blue-600 px-5 py-3 font-black hover:bg-blue-500 disabled:bg-zinc-700">Cadastrar produto</button></form></section>

    <section className="mt-10 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900"><div className="border-b border-zinc-800 px-6 py-5"><h2 className="text-xl font-black">Produtos no banco</h2></div>{products.length === 0 ? <p className="p-6 text-zinc-400">Nenhum produto cadastrado no banco ainda.</p> : <div className="divide-y divide-zinc-800">{products.map((p) => <div key={p.id} className="grid gap-3 px-6 py-5 md:grid-cols-[1fr_auto_auto_auto] md:items-center"><div><p className="font-bold">{p.name}</p><p className="text-sm text-zinc-500">{p.category}</p></div><p className="font-black text-blue-400">{money.format(p.price)}</p><AdminProductImageButton id={p.id} name={p.name}/><AdminDeleteButton id={p.id} name={p.name}/></div>)}</div>}</section>
  </main>;
}
