...
export default async function AdminPage() {
  ...
  return <main className="mx-auto max-w-7xl px-6 py-12">
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-black uppercase tracking-widest text-blue-400">Vitória Informática</p><h1 className="mt-2 text-4xl font-black">Painel administrativo</h1><p className="mt-3 text-zinc-400">Cadastre e gerencie os produtos do catálogo.</p></div><div className="flex flex-wrap gap-2"><Link href="/admin/clientes" className="rounded-xl border border-blue-500/40 px-5 py-3 font-bold text-blue-300 hover:bg-blue-500/10">Clientes</Link><Link href="/admin/pedidos" className="rounded-xl bg-blue-600 px-5 py-3 font-bold hover:bg-blue-500">Pedidos e orçamentos</Link><Link href="/produtos" className="rounded-xl border border-zinc-700 px-5 py-3 font-bold">Ver catálogo</Link><form action="/api/admin/logout" method="post"><button className="rounded-xl border border-red-500/30 px-5 py-3 font-bold text-red-300">Sair</button></form></div></div>
    ...
    <section className="mt-10 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900"><div className="border-b border-zinc-800 px-6 py-5"><h2 className="text-xl font-black">Produtos no banco</h2></div>{products.length === 0 ? <p className="p-6 text-zinc-400">Nenhum produto cadastrado no banco ainda.</p> : <div className="divide-y divide-zinc-800">{products.map((p) => <div key={p.id} className="grid gap-4 px-6 py-5 lg:grid-cols-[72px_1fr_auto_auto] lg:items-center"><div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950">{p.image ? <img src={p.image} alt={p.name} className="h-full w-full object-contain p-1"/> : <span className="text-2xl">🖥️</span>}</div><div><p className="font-bold">{p.name}</p><p className="text-sm text-zinc-500">{p.category} • {p.images?.length ?? 0} foto(s) • <strong className={(p.stockQuantity ?? 0) <= 2 ? "text-amber-400" : "text-green-400"}>{p.stockQuantity ?? 0} un.</strong></p><p className="mt-1 font-black text-blue-400">{money.format(p.price)}</p></div><Link href={`/admin/produtos/${encodeURIComponent(p.id)}/editar`} className="rounded-xl border border-blue-500/30 px-4 py-2 text-center text-sm font-bold text-blue-300">Editar</Link><AdminDeleteButton id={p.id} name={p.name}/></div>)}</div>}</section>
  </main>;
}
