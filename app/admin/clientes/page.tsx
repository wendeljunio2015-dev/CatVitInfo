    ...
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12">
      <div className="flex flex-wrap items-end justify-between gap-5">
        <div className="min-w-0">
          ...
          <h1 className="mt-2 text-3xl font-black sm:text-4xl">Clientes</h1>
          <p className="mt-3 text-zinc-400">Cadastre contatos e acompanhe o histórico de orçamentos.</p>
        </div>
        <Link href="/admin" className="rounded-lg border border-zinc-700 px-3 py-2 text-sm font-bold hover:bg-zinc-800">← Painel</Link>
      </div>
          ...
          <div className="divide-y divide-zinc-800">
            {customers.map((customer: any) => (
              <div key={String(customer.id)} className="grid gap-4 px-6 py-5 lg:grid-cols-[1fr_auto] lg:items-center">
                ...
                <Link href={`/admin/clientes/${encodeURIComponent(String(customer.id))}/editar`} className="rounded-lg border border-blue-500/30 px-3 py-2 text-center text-sm font-bold text-blue-300">Editar</Link>
              </div>
            ))}
          </div>
        )}
