...
export default async function EditProductPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12">
      ...
      {saved === "1" ? <div className="mt-6 rounded-2xl border border-green-500/20 bg-green-500/10 p-4 font-bold text-green-300">Produto atualizado com sucesso.</div> : null}
      ...
      <form action={`/api/admin/products/${encodeURIComponent(product.id)}`} method="post" encType="multipart/form-data" className="mt-8 grid gap-5 rounded-3xl border border-zinc-800 bg-zinc-900 p-6 md:grid-cols-2 md:p-8">
        <div>
          <label className="text-sm font-bold text-zinc-300">Nome do produto</label>
          <input required name="name" defaultValue={product.name} className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3" />
        </div>
        <div>
          <label className="text-sm font-bold text-zinc-300">Categoria</label>
          <select required name="category" defaultValue={product.category} className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3">
            {productCategories.map((category) => <option key={category} value={category}>{category}</option>)}
          </select>
        </div>
        <div>
          <label className="text-sm font-bold text-zinc-300">Preço</label>
          <input required name="price" type="number" min="0" step="0.01" defaultValue={product.price} className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3" />
        </div>
        <div>
          <label className="text-sm font-bold text-zinc-300">Garantia</label>
          <input name="warranty" defaultValue={product.warranty ?? ""} placeholder="Ex.: 3 meses" className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3" />
        </div>
        <div>
          <label className="text-sm font-bold text-zinc-300">Estoque</label>
          <select name="stockStatus" defaultValue={product.stockStatus} className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3">
        ...
        <div>
          <label className="text-sm font-bold text-zinc-300">Selo</label>
          <select name="badge" defaultValue={product.badge ?? ""} className="mt-2 w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3">
        ...
        <div className="flex flex-wrap gap-3 md:col-span-2">
          <button className="rounded-xl bg-blue-600 px-6 py-3 font-black hover:bg-blue-500">Salvar alterações</button>
          <Link href={`/produto/${product.slug}`} target="_blank" className="rounded-xl border border-zinc-700 px-6 py-3 font-bold">Ver produto</Link>
        </div>
      </form>
    </main>
  );
