...
export default async function OrdersPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const orders = await db.sql`
    SELECT id, order_number, customer_id, customer_name, items, total, status, stock_deducted_at,
           seller_id, seller_name, seller_commission_rate, commission_amount, created_at
  ...
  return <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12">
    <div className="flex flex-wrap items-end justify-between gap-5"><div className="min-w-0"><p className="text-sm font-black uppercase tracking-widest text-blue-400">Vitória Informática</p><h1 className="mt-2 text-3xl font-black sm:text-4xl">Pedidos e orçamentos</h1><p className="mt-3 text-zinc-400">Acompanhe vendas, vendedores e comissões.</p></div><div className="flex flex-wrap items-center gap-2 text-sm"><Link href="/admin/vendedores" className="rounded-lg border border-green-500/30 px-3 py-2 font-bold text-green-300">Vendedores</Link><Link href="/admin/clientes" className="rounded-lg bg-blue-600 px-3 py-2 font-bold hover:bg-blue-500">Clientes</Link><Link href="/admin" className="rounded-lg border border-zinc-700 px-3 py-2 font-bold hover:bg-zinc-800">← Painel</Link></div></div>
    ...
    <div className="mt-10 grid gap-5 sm:grid-cols-3"><div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6"><p className="text-sm text-zinc-400">Aguardando atendimento</p><p className="mt-2 text-4xl font-black text-amber-400">{openCount}</p></div><div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6"><p className="text-sm text-zinc-400">Concluídos</p><p className="mt-2 text-4xl font-black text-green-400">{completedCount}</p></div><div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6"><p className="text-sm text-zinc-400">Valor orçado</p><p className="mt-2 text-3xl font-black text-blue-400">{money.format(totalQuoted)}</p></div></div>
    <section className="mt-10 space-y-4">{orders.length === 0 ? <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-8 text-zinc-400">Nenhum orçamento registrado ainda.</div> : orders.map((order: any) => { const items = Array.isArray(order.items) ? order.items : []; const stockDeducted = Boolean(order.stock_deducted_at); return <article key={String(order.id)} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
      ...
      <form action={`/api/admin/orders/${encodeURIComponent(String(order.id))}`} method="post" className="mt-5 flex flex-wrap items-center gap-2"><select name="status" defaultValue={String(order.status)} className="rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm font-bold"><option value="novo">Novo</option><option value="em_atendimento">Em atendimento</option><option value="concluido">Concluído</option><option value="cancelado">Cancelado</option></select><button className="rounded-lg bg-blue-600 px-3 py-2 text-sm font-black hover:bg-blue-500">{stockDeducted ? "Atualizar status" : "Atualizar / confirmar venda"}</button>{!stockDeducted ? <span className="text-xs text-zinc-500">Ao concluir, baixa estoque e calcula a comissão uma única vez.</span> : <span className="text-xs text-green-400">Venda concluída. Ao cancelar, o sistema restaura o estoque e reverte a comissão; se houver comissão já paga no mês, o cancelamento automático será bloqueado para revisão.</span>}</form>
    </article>; })}</section>
  </main>;
}
