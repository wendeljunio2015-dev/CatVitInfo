  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-sm font-black uppercase tracking-widest text-blue-400">Vitória Informática</p><h1 className="mt-2 text-4xl font-black">Pedidos e orçamentos</h1><p className="mt-3 text-zinc-400">Acompanhe os orçamentos enviados pelo catálogo para o WhatsApp.</p></div>
        <div className="flex flex-wrap gap-2"><Link href="/admin/clientes" className="rounded-xl bg-blue-600 px-5 py-3 font-bold hover:bg-blue-500">Clientes</Link><Link href="/admin" className="rounded-xl border border-zinc-700 px-5 py-3 font-bold">← Voltar ao painel</Link></div>
      </div>
      ...
      <section className="mt-10 space-y-4">
        {orders.length === 0 ? <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-8 text-zinc-400">Nenhum orçamento registrado ainda.</div> : orders.map((order: any) => {
          ...
          return <article key={String(order.id)} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
            ...
            <form action={`/api/admin/orders/${encodeURIComponent(String(order.id))}`} method="post" className="mt-5 flex flex-wrap gap-3"><select name="status" defaultValue={String(order.status)} className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 font-bold"><option value="novo">Novo</option><option value="em_atendimento">Em atendimento</option><option value="concluido">Concluído</option><option value="cancelado">Cancelado</option></select><button className="rounded-xl bg-blue-600 px-5 py-3 font-black hover:bg-blue-500">Atualizar status</button></form>
          </article>;
        })}
      </section>
    </main>
