    ...
    <main className="mx-auto max-w-7xl px-6 py-12">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-4xl font-black">Pedidos e orçamentos</h1>
          <p className="mt-3 text-zinc-400">Acompanhe os orçamentos enviados pelo catálogo para o WhatsApp.</p>
        </div>
      ...
      <section className="mt-10 space-y-4">
        {orders.length === 0 ? (
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-8 text-zinc-400">Nenhum orçamento registrado ainda.</div>
        ) : orders.map((order: any) => {
          const items = Array.isArray(order.items) ? order.items : [];
          return (
            <article key={String(order.id)} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  <div className="flex flex-wrap items-center gap-3">
                    <h2 className="text-xl font-black">{String(order.order_number)}</h2>
                    <span className={`rounded-full border px-3 py-1 text-xs font-bold ${statusClass[String(order.status)] || "border-zinc-700 text-zinc-300"}`}>{statusLabel[String(order.status)] || String(order.status)}</span>
                  </div>
                  <p className="mt-2 text-sm text-zinc-400">{order.customer_name ? `Cliente: ${String(order.customer_name)} • ` : ""}{date.format(new Date(String(order.created_at)))}</p>
                </div>
                <strong className="text-2xl text-blue-400">{money.format(Number(order.total))}</strong>
              </div>

              <div className="mt-5 space-y-2 rounded-xl bg-zinc-950 p-4">
                {items.map((item: any, index: number) => (
                  <div key={`${String(item.productId)}-${index}`} className="flex justify-between gap-4 text-sm">
                    <span>{Number(item.quantity)}x {String(item.name)}</span>
                    <span className="font-bold">{money.format(Number(item.subtotal))}</span>
                  </div>
                ))}
              </div>

              <form action={`/api/admin/orders/${encodeURIComponent(String(order.id))}`} method="post" className="mt-5 flex flex-wrap gap-3">
                <select name="status" defaultValue={String(order.status)} className="rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 font-bold">
                  <option value="novo">Novo</option>
                  <option value="em_atendimento">Em atendimento</option>
                  <option value="concluido">Concluído</option>
                  <option value="cancelado">Cancelado</option>
                </select>
                <button className="rounded-xl bg-blue-600 px-5 py-3 font-black hover:bg-blue-500">Atualizar status</button>
              </form>
            </article>
          );
        })}
