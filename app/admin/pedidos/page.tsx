...
export default async function OrdersPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const orders = await db.sql`
    SELECT id, order_number, customer_id, customer_name, items, total, status, stock_deducted_at, created_at
    ...
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12">
      ...
      <section className="mt-10 space-y-4">
        {orders.length === 0 ? <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-8 text-zinc-400">Nenhum orçamento registrado ainda.</div> : orders.map((order: any) => {
          ...
          const stockDeducted = Boolean(order.stock_deducted_at);
          return <article key={String(order.id)} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
            <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-3"><h2 className="text-xl font-black">{String(order.order_number)}</h2><span className={`rounded-full border px-3 py-1 text-xs font-bold ${statusClass[String(order.status)] || "border-zinc-700 text-zinc-300"}`}>{statusLabel[String(order.status)] || String(order.status)}</span>{stockDeducted ? <span className="rounded-full border border-green-500/20 bg-green-500/10 px-3 py-1 text-xs font-bold text-green-300">Estoque baixado</span> : null}</div><p className="mt-2 text-sm text-zinc-400">{order.customer_name ? <>Cliente: {order.customer_id ? <Link href={`/admin/clientes/${encodeURIComponent(String(order.customer_id))}/editar`} className="font-bold text-blue-300 hover:underline">{String(order.customer_name)}</Link> : String(order.customer_name)} • </> : null}{date.format(new Date(String(order.created_at)))}</p></div><strong className="text-2xl text-blue-400">{money.format(Number(order.total))}</strong></div>
