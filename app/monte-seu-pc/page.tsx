...
export default function MonteSeuPcPage() {
  ...
  const total = useMemo(
    () => Object.values(selection).reduce((sum, product) => sum + (product?.price ?? 0), 0),
    [selection],
  );
  ...
  const whatsappHref = useMemo(() => {
    const lines = selectedItems.length
      ? selectedItems.map((product) => `• ${product.name} - ${money.format(product.price)}`).join("\n")
        ...
        <section className="space-y-5">
          {componentGroups.map((group) => {
            const options = products.filter(
              (product) => group.categories.includes(product.category as never) && product.stockStatus !== "indisponivel",
            );
              ...
              <div key={group.key} className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-zinc-500">{chosen ? chosen.name : "Nenhum item selecionado"}</p>
                  </div>
                  {chosen && (
                    <button onClick={() => setSelection((current) => ({ ...current, [group.key]: undefined }))} className="text-sm font-bold text-zinc-400 hover:text-white">Remover</button>
                  )}
                </div>
                  ...
                  <div className="mt-4 grid gap-3">
                    {options.map((product) => {
                      ...
                      return (
                        <button
                          key={product.id}
                          onClick={() => setSelection((current) => ({ ...current, [group.key]: product }))}
                          className={`flex items-center justify-between gap-4 rounded-xl border px-4 py-4 text-left ${active ? "border-blue-500 bg-blue-500/10" : "border-zinc-800 bg-zinc-950 hover:border-zinc-700"}`}
                        >
                          <span>
                          ...
                          <span className="shrink-0 font-black text-blue-400">{money.format(product.price)}</span>
                        </button>
                      );
                    })}
                  </div>
                ) : (
                  <div className="mt-4 rounded-xl border border-dashed border-zinc-700 bg-zinc-950/60 px-4 py-5 text-sm text-zinc-500">Ainda não há produtos desta categoria cadastrados no catálogo.</div>
                )}
              </div>
            );
          })}
          ...
          <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
            ...
            <div className="mt-4 grid gap-3">
              {products.filter((product) => product.category === "Kits Upgrade" && product.stockStatus !== "indisponivel").map((product) => (
                <Link key={product.id} href={`/produto/${product.slug}`} className="flex items-center justify-between gap-4 rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-4 hover:border-blue-500/60">
            ...
            <div className="mt-5 space-y-3">
              {selectedItems.length ? selectedItems.map((product) => (
                <div key={product.id} className="flex justify-between gap-4 text-sm">
                  ...
                  <span className="shrink-0 font-bold">{money.format(product.price)}</span>
                </div>
              )) : <p className="text-sm leading-6 text-zinc-500">Escolha os componentes para começar seu orçamento.</p>}
            </div>

            <div className="my-5 border-t border-zinc-800" />
            <div className="flex items-end justify-between gap-4">
              <span className="text-sm text-zinc-400">Total estimado</span>
              <span className="text-2xl font-black text-blue-500">{money.format(total)}</span>
            </div>

            {incompatible ? (
              <div className="mt-5 rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm leading-6 text-red-300">Atenção: o processador selecionado usa {cpuSocket} e a placa-mãe usa {motherboardSocket}. Essa combinação parece incompatível.</div>
            ) : cpuSocket && motherboardSocket ? (
              <div className="mt-5 rounded-xl border border-green-500/20 bg-green-500/10 p-4 text-sm text-green-300">Socket compatível identificado: {cpuSocket}.</div>
            ) : (
              <div className="mt-5 rounded-xl border border-zinc-800 bg-zinc-950 p-4 text-xs leading-5 text-zinc-500">A compatibilidade automática é uma ajuda inicial. A Vitória Informática confirma a configuração antes da venda.</div>
            )}
