...
export async function POST(request: Request) {
  try {
    ...
    const customerPhone = normalizePhone(String(body.customerPhone || "")) || null;
    const customerEmail = String(body.customerEmail || "").trim().toLowerCase().slice(0, 180) || null;
    ...
    await db.sql`
      INSERT INTO orders (id, order_number, customer_id, customer_name, items, total, status, source)
      VALUES (${id}, ${orderNumber}, ${customerId}, ${customerName}, ${JSON.stringify(items)}::jsonb, ${total}, 'novo', 'whatsapp')
    `;
