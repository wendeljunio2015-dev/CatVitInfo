...
function stockStatusFor(quantity: number) {
  if (quantity <= 0) return "indisponivel";
  if (quantity <= 2) return "ultimas_unidades";
  return "em_estoque";
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const existingImages = Array.isArray(current.images) ? current.images.map(String).filter(Boolean) : current.image ? [String(current.image)] : [];
    ...
    if (keptImages.length + newFiles.length > 5) return NextResponse.json({ error: "O produto pode ter no máximo 5 fotos." }, { status: 400 });
    ...
    if (requestedPrimary && images.includes(requestedPrimary)) images = [requestedPrimary, ...images.filter((url) => url !== requestedPrimary)];
    ...
    const stockStatus = stockStatusFor(stockQuantity);
    ...
    await db.sql`
      ...
      images = ${JSON.stringify(images)}::jsonb, updated_at = NOW() WHERE id = ${id}
    `;
