  ...
  return (
    <main className="mx-auto max-w-5xl px-6 py-12">
      ...
      <aside className="mt-8 rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
        ...
        {!paymentOrder && items.length ? <div className="mt-6 grid gap-4 md:grid-cols-2"><div className="rounded-2xl border border-blue-500/20 bg-blue-500/5 p-4"><p className="font-black text-blue-300">Pagar pelo Mercado Pago</p><p className="mt-1 text-sm text-zinc-400">Pix ou cartão com processamento seguro pelo Mercado Pago. Após aprovação, o estoque é baixado automaticamente.</p><button onClick={checkoutMercadoPago} disabled={sending || !mercadoPagoPublicKey} className="mt-4 w-full rounded-xl bg-blue-600 px-5 py-4 font-black disabled:bg-zinc-700">{sending ? "Preparando Mercado Pago..." : "Pagar pelo Mercado Pago"}</button></div><div className="rounded-2xl border border-green-500/20 bg-green-500/5 p-4"><p className="font-black text-green-300">Somente orçamento</p><p className="mt-1 text-sm text-zinc-400">Envie itens e quantidades pelo WhatsApp para negociar desconto e condições. Não confirma compra e não baixa estoque.</p><button onClick={checkoutWhatsApp} disabled={sending} className="mt-4 w-full rounded-xl bg-green-600 px-5 py-4 font-black disabled:bg-zinc-700">Solicitar orçamento no WhatsApp</button></div></div> : null}
