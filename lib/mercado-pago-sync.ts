...
export async function syncMercadoPagoPayment(payment: any, expectedOrderId?: string) {
  ...
  try {
    ...
    if (Math.abs(paidAmount - Number(order.total || 0)) > 0.009) {
    ...
    if (status === "approved") {
      if (!order.stock_deducted_at) {
        ...
        for (const item of items) {
          ...
          if (!productResult.rows.length || Number(productResult.rows[0].stock_quantity) < quantity) {
            stockIssue = true;
            break;
          }
        ...
        if (stockIssue) {
          ...
          await client.query(
            "UPDATE orders SET status = $1, source = 'mercado_pago', updated_at = NOW() WHERE id = $2",
